...
export default async function ProjectBox({ params: { lng }, id, name, url, desc, stack, imgurl }: { params: { lng: string }; id: string; name: string; url: string; desc: string; stack: string[]; imgurl: string }) {
  ...
  return (
    <div className='w-full h-full md:border-b-0 border-b-2 border-l-violet12'>
      ...
      <div className="w-full flex flex-col md:flex-row justify-between items-center md:border-b-2 border-b-0 border-violet12">
        ...
        hover:bg-violet9 hover:text-violet1`}>{t('projects__deployBtn')}</Link>
        </div>
      </div>
    </div>
  )
